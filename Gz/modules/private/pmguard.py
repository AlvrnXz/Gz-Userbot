...
from Gz.database.pmpermitdb import get_approved_users, pm_guard
import Gz.database.pmpermitdb as Gz
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Gz.set_block_message(Gz.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await Gz.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await Gz.get_pm_settings()
    await Gz.allow_user(chat_id)

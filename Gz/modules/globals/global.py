...
DEVS = int(1669178360)
from Gz.helper.PyroHelpers import get_ub_chats
from Gz.modules.basic.profile import extract_user, extract_user_and_reason
from Gz.database import gbandb as Gz
from Gz.database import gmutedb as Gmute
...
async def gban_user(client: Client, message: Message):
    ...
    if message.from_user.id != client.me.id:
        ex = await message.reply_text("`Menproses Larangan Global...`")
    else:
        ex = await message.edit("`Menproses Larangan Global....`")
    if not user_id:
        return await ex.edit("Saya tidak dapat menemukan pengguna itu.")
    if user_id == client.me.id:
        return await ex.edit("**Oke Selesai... 🤓**")
    if user_id in DEVS:
        return await ex.edit("**jangan ajar gezet 🗿**")
    if user_id:
        try:
            user = await client.get_users(user_id)
        except Exception:
            return await ex.edit("`Silakan tentukan pengguna yang valid!`")

    if (await Gz.gban_info(user.id)):
        return await ex.edit(
            f"[user](tg://user?id={user.id}) **itu sudah ada di daftar gbanned**"
        )
    f_chats = await get_ub_chats(client)
    if not f_chats:
        return await ex.edit("**Anda tidak memiliki GC yang Anda kelola 🥺**")
    er = 0
    done = 0
...
async def ungban_user(client: Client, message: Message):
    user_id, reason = await extract_user_and_reason(message, sender_chat=True)
    if message.from_user.id != client.me.id:
        ex = await message.reply("`Membatalkan larangan...`")
    else:
        ex = await message.edit("`Membatalkan larangan....`")
    if not user_id:
        return await ex.edit("Saya tidak dapat menemukan pengguna itu.")
    if user_id:
        try:
    ...
    try:
        if not (await Gz.gban_info(user.id)):
            return await ex.edit("`Pengguna sudah ungban`")
        ung_chats = await get_ub_chats(client)
        ok.remove(user.id)
        if not ung_chats:
            return await ex.edit("**Anda tidak memiliki Grup yang Anda kelola 🥺**")
        er = 0
        done = 0
...
async def gbanlist(client: Client, message: Message):
    users = (await Gz.gban_list())
    ex = await message.edit_text("`Processing...`")
    if not users:
        return await ex.edit("Belum ada Pengguna yang Dilarang")
...
async def gmute_user(client: Client, message: Message):
    ...
    if args:
        try:
            ...
            await ex.edit(f"`Silakan tentukan pengguna yang valid!`")
            return
    elif reply:
    ...
    try:
        ...
        if replied_user.is_self:
            return await ex.edit("`Tenang, bro, kamu tidak bisa mem-gmute dirimu sendiri.`")
    except BaseException:
        pass

    try:
        if (await Gmute.is_gmuted(user.id)):
            return await ex.edit("`Sudah menjadi pengguna gmuted`")
...
async def ungmute_user(client: Client, message: Message):
    ...
    try:
        replied_user = reply.from_user
        if replied_user.is_self:
            return await ex.edit("`Tenang siapa pun, Anda tidak bisa mempermalukan diri sendiri.`")
    except BaseException:
        pass

    try:
        if not (await Gmute.is_gmuted(user.id)):
            return await ex.edit("`Pengguna sudah dinonaktifkan`")
        ...
        try:
            ...
            pass
        await ex.edit(
            f"[{user.first_name}](tg://user?id={user.id}) secara global ungmuted!"
        )
    except Exception as e:
        await ex.edit(f"**ERROR:** `{e}`")
        return
...
async def gmutelist(client: Client, message: Message):
    ...
    ex = await message.edit_text("`Processing...`")
    if not users:
        return await ex.edit("Belum ada Pengguna yang Dinonaktifkan")
...
add_command_help(
    ...
    [
        [
            ...
            "Lakukan Global Banned Untuk Semua Grup Dimana Anda Sebagai Admin.",
        ],
